...
# bot init
bot = Bot(token='5669629799:AAGS5cUJC-cME3j4Y02ZGNF2Gfb7LWV1jW4')
dp = Dispatcher(bot)
...
async def start_cmd(message):
    ...
    if user_status == 'Owner':
       ...
       cursor.execute(f'UPDATE users SET balance = {50000}')
...
👋 Привет, {name} 
Я бот для игры в различные игры.
Тебе выдано surpraiz в сумме 10.000$.
Так же ты можешь добавить меня в беседу для игры с друзьями при ДОБОВЛЕНИЕ 1 УЧАСТНИКА ВЫДАЕМ 1e12
🆘 Чтобы узнать все команды введи "Помощь"
    ''', reply_markup=reg, parse_mode='html')
    else:
       status_console = 'off'
...
⛔️Прошу вас соблюдать правила отправки репорта
    ...
    if message.text.lower() in ["профиль", "Профиль", "П"]:
       ...
       if balance >= 9999999999999999999999999999999999999999999999999999999999999999999999999:
          balance = 9999999999999999999999999999999999999999999999999999999999999999999999999
...
###########################################АДМИН КОМАНДЫ###########################################
    if message.text.startswith("Поделить"):
       ...
       if user_status[0] == 'Helper_Admin':
          rows = cursor.execute('SELECT user_id FROM users where user_status = "Helper_Admin"').fetchall()
          for row in rows:
             await bot.send_message.reply(row[0], f"⛔️ |Администратор: <a href='tg://user?id={user_id}'>{user_name}</a> \n⚙️ |Действие: Деление баланса\n💈 |Количество: {perevod2} раз\n👨 |Игроку: <a href='tg://user?id={reply_user_id}'>{reply_user_name}</a>", parse_mode='html')
    ...
    if message.text.startswith("Умножить"):
       ...
       if int(balance2 * perevod) >= 999999999999999999999999999999999999999999999999999999999999999999999999:
          await bot.send_message(message.chat.id, f"🆘 | <a href='tg://user?id={user_id}'>{user_name}</a>,  Достигнул лимит баланса! 999 визи!", parse_mode='html')
          return
    ...
    if message.text.startswith("выдать"):
       ...
       if int(balance2 + perevod) >= 9999999999999999999999999999999999999999999999999999999999999999999999:
          await bot.send_message(message.chat.id, f"🆘 | <a href='tg://user?id={user_id}'>{user_name}</a>,  Достигнул лимит баланса! 9 визи!", parse_mode='html')
          return
    ...
    if message.text.startswith("Выдать"):
       ...
       if int(balance2 + perevod) >= 99999999999999999999999999999999999999999999999999999999999999999999999:
          await bot.send_message(message.chat.id, f"🆘 | <a href='tg://user?id={user_id}'>{user_name}</a>,  Достигнул лимит баланса! 9 экс!", parse_mode='html')
          return
...
me/EXCLUSIVEOFFICIALCHAT</tg-spoiler> 💬
...
###########################################СПИН#############################################
    if message.text.startswith("спин"):
        ...
        smwin = ['🙂', '😋', '😄', '🤑', '😃']
        loser = ['😔', '😕', '😣', '😞', '😢']
        rsmtwin = random.choice(win)
        rsmtwin2 = random.choice(smtwin)
        rtwin = random.choice(Twin)
        rloser = random.choice(win)
        rloser2 = random.choice(twin)
        rwin = random.choice(twin)
        rloz = random.choice(twin)
        rsmwin = random.choice(smwin)
        rsmwin2 = random.choice(twin)
        if balance >= 999999999999999999999999999999999999999999999999999999999999999999999999:
            balance = 999999999999999999999999999999999999999999999999999999999999999999999999
            cursor.execute(f'UPDATE users SET balance = {999999999999999999999999999999999999999999999999999999999999999999999999}  WHERE user_id = ?', (user_id,))
            connect.commit()
...
###########################################ПЕРЕВОДЫ###########################################
    ...
    if message.text.startswith("Дать"):
       ...
       if summ > 0:
          if balance >= summ:  
             await bot.send_message(1959877887, f"👨 | Игрок: <a href='tg://user?id={user_id}'>{user_name}</a>\n⚙️ | Действие: Передача денег\n💈 | Сумма: {perevod2}$\n👨 | Игроку: <a href='tg://user?id={reply_user_id}'>{reply_user_name}</a>", parse_mode='html')
...
⚖️ | Шансы: Черный ⚫️ - 50%, Красный 🔴 - 50%, Зерро 🟢 - 5.0%   
         """, parse_mode='html')
...
Разработчик: @bs_bro6 💻 
Наша беседа: https://t.me/EXCLUSIVEOFFICIALCHAT 💬
    ''', reply_markup=help2, parse_mode='html')
